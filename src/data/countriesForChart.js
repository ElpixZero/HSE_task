/* 
С помощью сервиса онлайн конвертации excel to Json (http://beautifytools.com/excel-to-json-converter.php) получил данные, которые находятся в массиве "IMRfromExcel".
Далее из-за определенного формата данных, который необходим для построения графика сгруппировал объекты, относящиеся к одной стране по три.
После данного группирования, преобразовал исходные данные в формат, который понимает компонент отрисовывающий график.
*/
...
function convertedDataFromExcelForLineChart(IMRfromExcel) {
  let convertedIMR= [];
  let teamArray = [];
  
  for (let i = 1;  i <= IMRfromExcel.length; i++) {
    teamArray.push(IMRfromExcel[i-1])
  
    if (i % 3 === 0 && i !== 0) {
      convertedIMR.push(teamArray);
      teamArray = [];
    }
  };

  return convertedIMR.map( item => {
    return {
...
export default convertedDataFromExcelForLineChart(IMRfromExcel)
